  ...
  mysqldesc(config, function(err, data) {
    if (err) {
      ...
      async.forEachOf(data, function(value, table, callback) {
        if (data.hasOwnProperty(table)) {
          ...
          mysqldesc.keyColumnUsage(config, config.database, table, function(err, data2) {
            for (var colum in tableCloumnsData) {
              ...
              if(attributes['Extra'] !== 'auto_increment' && attributes['Extra'] !== 'on update current_timestamp()' && attributes['Default'] !== 'current_timestamp()'){
        ...
        if (folder_routes !== "" && folder_routes) {
          saveRoutes(folder_routes, Models);
        }
        if (folder_links !== "" && folder_links) {
          saveLinks(folder_links, Models);
        }
...
exports.toSailsAttribute = function(Type, attrib, default_value_, is_nullable_, key_, reference_fk) {
  ...
  if (default_value_ !== "" && default_value_ !== null) {
    var def = "defaultsTo: ";
    if (content_view.type == "text" || content_view.type == "date") {
      def += `"${default_value_}"`;
    } else {
      def += default_value_;
    }
